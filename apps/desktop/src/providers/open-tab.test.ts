...
describe('openTabForRoute', () => {
  it('opens a note tab for ordinary notes and a surface tab for screens', () => {
  ...
  it('leaves Daily and search off the strip', () => {
...
describe('routeForOpenTab', () => {
  it('round-trips notes and surfaces to their canonical routes', () => {
    ...
    expect(routeForOpenTab({ kind: 'surface', surface: 'allNotes', pinned: false })).toEqual({
      kind: 'allNotes',
      tag: null,
    })
  })
