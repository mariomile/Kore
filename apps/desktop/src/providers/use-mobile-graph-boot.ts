 ...
 * `iCloud Drive → Kore → Notes` in Files/Finder. */
    ...
    void (async () => {
      try {
        ...
        if (root === null) {
          ...
          // iCloud is back.
          onParked(
            'Your notes are stored in iCloud Drive, but iCloud isn’t available on this device. Sign in to iCloud in Settings, then reopen Kore.',
          )
          return
        }
