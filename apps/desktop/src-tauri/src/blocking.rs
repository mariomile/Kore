...
mod tests {
    ...
    const SYNC_COMMAND_ALLOWLIST: &[(&str, &str, &str)] = &[
        ...
        (
            ...
            "background_task.rs",
            "flips an in-memory counter under a mutex",
        ),
        (
            "background_task_end",
            "background_task.rs",
        ...
        (
            "browser_embed_back",
            "browser.rs",
            "owns webview lifetime and geometry, which AppKit requires on the main thread",
        ),
        (
            "browser_embed_back",
            "browser_mobile.rs",
        ...
        (
            "embed_status",
            "embed.rs",
            "reads cached model status behind a mutex",
        ),
        (
            "embed_status",
            "embed_mobile.rs",
        ...
        (
            "haptic_feedback",
            "haptics.rs",
            "fires a native haptic, a UI-thread call that returns immediately",
        ),
        (
            "icloud_watch_stop",
            "icloud/watch/mod.rs",
            "stops a watcher by flipping an atomic; the watcher itself runs off-thread",
        ),
        (
            "menu_install_paste_and_match_style",
            "menu.rs",
            "installs a native menu item, which AppKit requires on the main thread",
        ),
        (
        ...
        (
            "pty_ack",
            "pty.rs",
            "writes to an already-open PTY handle: a buffered fd write, not a file open",
        ),
        (
            "pty_ack",
            "pty_mobile.rs",
        ...
        (
            "quick_capture_hide",
            "windows.rs",
            "window and registry state under a mutex; no filesystem or SQLite work",
        ),
        (
            "quit_confirm",
            "quit.rs",
            "drives the native quit dialog on the main thread",
        ),
        (
            "recent_graphs",
            "recents.rs",
        ...
        (
            "watch_start",
            "watcher.rs",
            "starts and stops the notify watcher; the watch loop itself runs off-thread",
        ),
        (
            "watch_start",
            "watcher_mobile.rs",
