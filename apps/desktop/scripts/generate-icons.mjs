...
import { SOLAR_ICONS, TAG_SYMBOL_ICON_GROUPS, TAG_SYMBOL_ICONS } from './icon-manifest.mjs'

const require = createRequire(import.meta.url)
const scriptDir = import.meta.dirname
const outputPath = join(scriptDir, '../src/components/icons/solar-icons.gen.tsx')
const symbolsOutputPath = join(scriptDir, '../src/lib/tags/tag-symbol-icons.gen.tsx')
const catalogOutputPath = join(
  scriptDir,
  '../../../packages/core/src/tags/tag-symbol-catalog.gen.ts',
)
...
// The tag symbol table: stored name → icon, same family, own file so the
// app's named icon exports stay the app's.
const symbolForeign = Object.entries(TAG_SYMBOL_ICONS).filter(
  ([, iconId]) => !iconId.endsWith('-linear'),
)
if (symbolForeign.length > 0) {
  const names = symbolForeign.map(([name, iconId]) => `${name} (${iconId})`).join(', ')
  throw new Error(`Only Solar's -linear family is in the set; drop or repoint: ${names}`)
}
const symbolEntries = Object.entries(TAG_SYMBOL_ICONS).map(([name, iconId]) => {
  const { body } = resolveIcon(iconSet, iconId)
  return [
    `  /** Solar \`${iconId}\`. */`,
    `  '${name}': createIcon(`,
...
const symbolsFile = [
  '// Generated by scripts/generate-icons.mjs — do not edit by hand.',
  `// Source: @iconify-json/solar@${solarVersion} (Solar icon set, CC BY 4.0).`,
  `// Add a symbol in scripts/icon-manifest.mjs (TAG_SYMBOL_ICONS), then re-run the script.`,
  '',
  `import { createIcon, type Icon } from '@/components/icons/create-icon'`,
  '',
  '/** Stored symbol name → icon, in manifest (picker) order. */',
  'export const TAG_SYMBOL_ICONS: Readonly<Record<string, Icon>> = {',
  symbolEntries.join('\n'),
  '}',
  '',
].join('\n')
writeFileSync(symbolsOutputPath, symbolsFile)
console.log(`Wrote ${symbolEntries.length} tag symbols to ${symbolsOutputPath}`)

// The catalog the chat model reads (and the tools validate against): the
// same stored names, each with a short hint made of its group and the words
// of its Solar id. No SVG — core never renders, it only names.
const catalogEntries = Object.entries(TAG_SYMBOL_ICON_GROUPS).flatMap(([group, icons]) =>
  Object.entries(icons).map(([name, iconId]) => {
    const words = iconId.replace(/-linear$/, '').replaceAll('-', ' ')
    const hint = words === name.replaceAll('-', ' ') ? group : `${group} · ${words}`
    return `  { name: '${name}', hint: '${hint}' },`
  }),
)
const catalogFile = [
  '// Generated by apps/desktop/scripts/generate-icons.mjs — do not edit by hand.',
  `// Source: apps/desktop/scripts/icon-manifest.mjs (TAG_SYMBOL_ICON_GROUPS), @iconify-json/solar@${solarVersion}.`,
...
writeFileSync(catalogOutputPath, catalogFile)
console.log(`Wrote ${catalogEntries.length} catalog entries to ${catalogOutputPath}`)
