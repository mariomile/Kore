...
import { DEFAULT_SETTINGS, setBridge, type AiProviderConfig } from '@reflect/core'
...
describe('SettingsProvider', () => {
  ...
  it('an equal-but-rebuilt array value does not trigger a save', async () => {
    ...
    await vi.waitFor(() =>
      expect(saved).toEqual([{ ...DEFAULT_SETTINGS, allNotesFilterTags: ['book'] }]),
    )
  })
  ...
  it('applies an update instantly and persists the full document', async () => {
    ...
    await vi.waitFor(() =>
      expect(saved).toEqual([
        { ...DEFAULT_SETTINGS, editorMarkdownSyntax: 'show', futureKey: true },
      ]),
    )
  })
