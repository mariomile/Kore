...
- **Shell:** Tauri 2's iOS target generated from `ios.project.yml` (XcodeGen) into `gen/apple/`. Template edits require `tauri ios init` regeneration. The CLI sidecar is excluded from mobile bundles by the platform overlays. App identity is `app.lore.ios` / product name "Reflect".
