...
describe('NoteEditor slash menu', () => {
  it('groups the slash menu into Notion-style sections', async () => {
    ...
    const heading1 = page.locate('[data-testid="slash-menu"] [value="Heading 1 h1"]')
  ...
  it('keeps consecutive slash rows from overlapping', async () => {
    ...
    await userEvent.keyboard('/heading')
    const heading1 = page.locate('[data-testid="slash-menu"] [value="Heading 1 h1"]:not([hidden])')
    const heading2 = page.locate('[data-testid="slash-menu"] [value="Heading 2 h2"]:not([hidden])')
    await expect.element(heading1).toBeVisible()
