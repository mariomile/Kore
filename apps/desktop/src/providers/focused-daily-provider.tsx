...
/**
 * Clear the focused day whenever this pane arrives somewhere: what restores
 * the routed day is navigation, not blur, so focus deliberately *stays*
 * through transient moves (opening ⌘K, clicking a sidebar button) rather than
 * flicking back and out again. The reset keys off the same signals the stream
 ...
 * pre-paint (a layout effect) so no stale day shows before the stream
 * re-focuses the target.
 *
 * It belongs to the pane that owns the router and the focused-day store, not
 * to the chrome that reads them: the chrome rebinds to whichever pane is
 * active, and resetting there would wipe the focused day of the pane the user
 * just switched to.
 */
export function useResetFocusedDailyOnArrival(): void {
...
/** Mounts {@link useResetFocusedDailyOnArrival} inside a pane's providers. */
export function FocusedDailyArrivalReset(): null {
 ...
 * in the daily stream. Read-only: the reset lives in
 * {@link useResetFocusedDailyOnArrival}.
 *
 ...
 * {@link effectiveDailyDate} precedence the note-scoped commands share. With
 * nothing focused it is just the routed subject.
 */
