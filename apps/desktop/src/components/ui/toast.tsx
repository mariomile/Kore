...
import { AlertTriangle, CheckCircle, Close, CloseCircle, Info, Spinner } from '@/components/icons'
...
function Toast({
  ...
  return (
    <ToastPrimitive.Root
      data-slot="toast"
      className={cn(
        'group/toast pointer-events-auto absolute right-0 z-[calc(1000-var(--toast-index))] w-full rounded-2xl border bg-popover text-popover-foreground shadow-pop will-change-transform outline-none select-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50',
...
function ToastClose({
  ...
  return (
    <ToastPrimitive.Close
      ...
      {...props}
    >
      {children ?? <Close aria-hidden="true" />}
    </ToastPrimitive.Close>
  )
}
...
function ToastIcon({ type }: { type: string | undefined }): ReactElement | null {
  ...
  if (type === 'error') {
    icon = <CloseCircle className="text-destructive" aria-hidden="true" />
  }

  if (type === 'loading') {
    icon = <Spinner className="animate-spin" aria-hidden="true" />
  }
